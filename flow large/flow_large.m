...
img_fancy = imread("img_large.png");
...
grid_x = single(grid_x);
grid_y = single(grid_y);

[pressure_field,v_x,v_y] = deal( single( zeros(scene_scale(1), scene_scale(2)) ) );

solid_mask = rand(scene_scale(1), scene_scale(2)) < 0.15; %adds turbulent-generating edges
...
%regarding particles
particle_index = 1;
particle_sourcemask_x = single([]);
particle_sourcemask_y = single([]);
...
while iter <= maxiters
    ...
    v_divergence = single( divergence(v_x, v_y)/2 );

    p_p = single( ones([size(v_x)]) );
    d_p = single( ones([size(v_x)]) );
    
    p_tmpfield = single( zeros(scene_scale(1), scene_scale(2)) );

    pressure_solve_iters = 1;
    while max(max(d_p)) > 9e-3

        p_tmpfield_prev = single(pressure_field);
        ...
        pressure_field(pressure_in_mask) = 3;
    ...
    v_x = single( interp2(v_x, pv_x, pv_y, 'linear', 0) );
    v_y = single( interp2(v_y, pv_x, pv_y, 'linear', 0) );
    ...
    for n=1:2
        particlelist_x = single( [particle_sourcemask_x + rand(height(particle_sourcemask_x),1)*emission_turbulance_factor; particlelist_x] );
        particlelist_y = single( [particle_sourcemask_y + rand(height(particle_sourcemask_y),1)*emission_turbulance_factor; particlelist_y] );
    end
    ...
    ind_colliding = single( find_colliding_particles(particlelist_x, particlelist_y, solid_mask) );
    ...
    if rem(iter,frame_rem_factor) == 0
        ...
        frame_current = single(plot_field);
        frame = frame+1;
