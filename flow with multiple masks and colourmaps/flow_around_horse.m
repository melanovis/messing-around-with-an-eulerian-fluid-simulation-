...
frame_rem_factor = 5; %higher is more simtime between frames
fps = 60;
shock_frames = []; %iters when shocks occur
...
for n=1:height(collider_mask)
    for m=1:width(collider_mask)
        if collider_mask(n,m) && rand() > 0.05
...
cmap = interp1([linspace(0,1,5)], [[0 0 0]; augmented_abyss], linspace(0, 1, 1e3));
...
scene_scale = [height(collider_mask),width(collider_mask)];
...
while iter <= maxiters
    ...
    for n=1:5 %how many particles are we adding?
    ...
    if rem(iter,frame_rem_factor) == 0
        ...
        if render_thing
            frame_total = frame_colourised;
            frame_total = frame_total + greyscale_img_solidmask(solid_mask);
            frame_total = frame_total + greyscale_img_solidmask(collider_mask)./5;
        ...
        frame = frame+1;
        save(filename,"frame_colourised","solid_mask","plot_field"); %overwrite current stored frame
