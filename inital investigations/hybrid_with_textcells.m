...
fps = 60;
texteffect_scale_factor = 8; %how many pixels for each char?

load("tiles_formatted.mat")

textcells_message = floor(rand(1,1e6)*16); %rand for now, real message later
textcells_message = string(dec2hex(textcells_message)).';

maxiters = fps * frame_rem_factor * 35;
...
while iter <= maxiters
    ...
    % max_p_delta = 1e3;
    % while max_p_delta > 1e-9
    %     for n = 2:scene_height-1
    %         for m = 2:scene_width-1
    %             p_tmpfield_prev = pressure_field(n,m);
    %             pressure_field(n,m) = 0.25 * (pressure_field(n+1,m) + ...
    %                                           pressure_field(n-1,m) + ...
    %                                           pressure_field(n,m+1) + ...
    %                                           pressure_field(n,m-1) - ...
    %                                           v_divergence(n,m)); %gauss seidel method
    %             p_delta = abs(pressure_field(n,m) - p_tmpfield_prev);
    ...
    if rem(iter,frame_rem_factor) == 0
        ...
        if render_test
            % frame = getframe(gcf);
            % test_write = return_map + plot_solidmask;
    ...
    for x=1:width(order_map)
        for y=1:height(order_map)
            if order_map(y,x) ~= 0
                ...
                % if rand() < 5e-3 && character_read~="D" %some random jitter (character D may clip)
