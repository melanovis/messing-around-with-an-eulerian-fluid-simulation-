...
if record_video == true
    v = VideoWriter("fluid_test", 'MPEG-4');
    v.FrameRate = 60;
    open(v);
end
...
micro_turbulance_factor = 1e-8;
grid_spacing = 1;
emission_turbulance_factor = 0.5; % 1 for complete intra pixel rand on the source mask
...
[grid_x, grid_y] = meshgrid(1:scene_scale*ar, 1:scene_scale);

pressure_field = zeros(scene_scale, scene_scale*ar);
v_x = zeros(scene_scale, scene_scale*ar);
v_y = zeros(scene_scale, scene_scale*ar);

solid_mask = zeros(scene_scale, scene_scale*ar);
for n=50:100
    solid_mask(50:150,n) = rand(1,abs(50-150)+1) < 0.8;
end
solid_mask = logical(solid_mask);

particle_inflow_mask = zeros(scene_scale, scene_scale*ar);
particle_inflow_mask(60:140,3) = 1;
...
while true
    ...
    while max(max(d_p)) > 1e-3
        pressure_field = (conv2(pressure_field, jacobi_mask, 'same') - v_divergence); %need to step away from conv() in the next iteration
    ...
    for n=1:4 %how many particles are we adding?
    ...
    %cull particles intersecting with solid
    ...
    if rem(iter,5) == 0
        ...
        plot_solidmask = single(solid_mask(4:end-3,4:end-3));
